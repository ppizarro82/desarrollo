...
class caudittrail_view extends caudittrail {
	...
	function RenderRow() {
		...
		// newvalue
		if (strval($this->newvalue->CurrentValue) <> "") {
			$this->newvalue->ViewValue = $this->newvalue->OptionCaption($this->newvalue->CurrentValue);
		} else {
			$this->newvalue->ViewValue = NULL;
		}
...
faudittrailview.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
faudittrailview.Lists["x_newvalue"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
faudittrailview.Lists["x_newvalue"].Options = <?php echo json_encode($audittrail_view->newvalue->Options()) ?>;
...
<input type="hidden" name="modal" value="<?php echo intval($audittrail_view->IsModal) ?>">
