...
$ciudades_view = NULL; // Initialize page object first

class cciudades_view extends cciudades {
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["Id"] <> "") {
				...
				$sReturnUrl = "ciudadeslist.php"; // Return to list
			}
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "ciudadeslist.php"; // No matching record, return to list
					}
			}
			...
			$sReturnUrl = "ciudadeslist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
...
// Create page object
if (!isset($ciudades_view)) $ciudades_view = new cciudades_view();

// Page init
$ciudades_view->Page_Init();

// Page main
$ciudades_view->Page_Main();
...
var CurrentForm = fciudadesview = new ew_Form("fciudadesview", "view");

// Form_CustomValidate event
fciudadesview.Form_CustomValidate = 
...
<?php
	foreach ($ciudades_view->OtherOptions as &$option)
		$option->Render("body");
?>
...
?>
<form name="fciudadesview" id="fciudadesview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($ciudades_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $ciudades_view->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="ciudades">
<input type="hidden" name="modal" value="<?php echo intval($ciudades_view->IsModal) ?>">
<table class="table table-striped table-bordered table-hover table-condensed ewViewTable">
<?php if ($ciudades->Id->Visible) { // Id ?>
	<tr id="r_Id">
		<td class="col-sm-2"><span id="elh_ciudades_Id"><?php echo $ciudades->Id->FldCaption() ?></span></td>
		<td data-name="Id"<?php echo $ciudades->Id->CellAttributes() ?>>
<span id="el_ciudades_Id">
<span<?php echo $ciudades->Id->ViewAttributes() ?>>
<?php echo $ciudades->Id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php if ($ciudades->estado->Visible) { // estado ?>
	<tr id="r_estado">
		<td class="col-sm-2"><span id="elh_ciudades_estado"><?php echo $ciudades->estado->FldCaption() ?></span></td>
...
<?php echo $ciudades->estado->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
