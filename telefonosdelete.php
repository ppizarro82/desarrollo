...
$telefonos_delete = NULL; // Initialize page object first

class ctelefonos_delete extends ctelefonos {
	...
	// Page object name
	var $PageObjName = 'telefonos_delete';
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("telefonoslist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in telefonos class, telefonosinfo.php
		...
		if ($this->CurrentAction == "I") { // Load records for display
			...
			if ($this->TotalRecs <= 0) { // No record found, exit
				...
				$this->Page_Terminate("telefonoslist.php"); // Return to list
			}
		}
	}
...
// Create page object
if (!isset($telefonos_delete)) $telefonos_delete = new ctelefonos_delete();

// Page init
$telefonos_delete->Page_Init();

// Page main
$telefonos_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$telefonos_delete->Page_Render();
?>
...
var CurrentForm = ftelefonosdelete = new ew_Form("ftelefonosdelete", "delete");

// Form_CustomValidate event
ftelefonosdelete.Form_CustomValidate = 
...
<?php $telefonos_delete->ShowPageHeader(); ?>
<?php
$telefonos_delete->ShowMessage();
?>
<form name="ftelefonosdelete" id="ftelefonosdelete" class="form-inline ewForm ewDeleteForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($telefonos_delete->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $telefonos_delete->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="telefonos">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($telefonos_delete->RecKeys as $key) { ?>
...
<?php } ?>
	...
	<tbody>
<?php
$telefonos_delete->RecCnt = 0;
$i = 0;
while (!$telefonos_delete->Recordset->EOF) {
	$telefonos_delete->RecCnt++;
	$telefonos_delete->RowCnt++;

	// Set row properties
	$telefonos->ResetAttrs();
	$telefonos->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$telefonos_delete->LoadRowValues($telefonos_delete->Recordset);

	// Render row
	$telefonos_delete->RenderRow();
?>
	<tr<?php echo $telefonos->RowAttributes() ?>>
<?php if ($telefonos->Id->Visible) { // Id ?>
		<td<?php echo $telefonos->Id->CellAttributes() ?>>
<span id="el<?php echo $telefonos_delete->RowCnt ?>_telefonos_Id" class="telefonos_Id">
...
<?php
	$telefonos_delete->Recordset->MoveNext();
}
$telefonos_delete->Recordset->Close();
?>
</tbody>
</table>
</div>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $telefonos_delete->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
</div>
</form>
<script type="text/javascript">
ftelefonosdelete.Init();
</script>
<?php
$telefonos_delete->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$telefonos_delete->Page_Terminate();
?>
