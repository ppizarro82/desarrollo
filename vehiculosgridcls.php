...
$vehiculos_grid = NULL; // Initialize page object first

class cvehiculos_grid extends cvehiculos {
	...
	// Page object name
	var $PageObjName = 'vehiculos_grid';

	// Grid form hidden field names
	var $FormName = 'fvehiculosgrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["vehiculos"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function EmptyRow() {
		...
		if ($objForm->HasValue("x_id_gestion") && $objForm->HasValue("o_id_gestion") && $this->id_gestion->CurrentValue <> $this->id_gestion->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_marca") && $objForm->HasValue("o_marca") && $this->marca->CurrentValue <> $this->marca->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_modelo") && $objForm->HasValue("o_modelo") && $this->modelo->CurrentValue <> $this->modelo->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_placa") && $objForm->HasValue("o_placa") && $this->placa->CurrentValue <> $this->placa->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_anio") && $objForm->HasValue("o_anio") && $this->anio->CurrentValue <> $this->anio->OldValue)
			return FALSE;
		return TRUE;
	}
