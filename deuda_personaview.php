...
$deuda_persona_view = NULL; // Initialize page object first

class cdeuda_persona_view extends cdeuda_persona {
	...
	// Page object name
	var $PageObjName = 'deuda_persona_view';
	...
	function __construct() {
		...
		if (@$_GET["id_deuda"] <> "") {
			$this->RecKey["id_deuda"] = $_GET["id_deuda"];
			$KeyUrl .= "&amp;id_deuda=" . urlencode($this->RecKey["id_deuda"]);
		}
	...
	function Page_Init() {
		...
		if (@$_GET["id_persona"] <> "") {
			...
			$gsExportFile .= $_GET["id_persona"];
		}
		if (@$_GET["id_deuda"] <> "") {
			if ($gsExportFile <> "") $gsExportFile .= "_";
			$gsExportFile .= $_GET["id_deuda"];
		}
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["id_persona"] <> "") {
				...
				$sReturnUrl = "deuda_personalist.php"; // Return to list
			}
			if (@$_GET["id_deuda"] <> "") {
				$this->id_deuda->setQueryStringValue($_GET["id_deuda"]);
				$this->RecKey["id_deuda"] = $this->id_deuda->QueryStringValue;
			} elseif (@$_POST["id_deuda"] <> "") {
				$this->id_deuda->setFormValue($_POST["id_deuda"]);
				$this->RecKey["id_deuda"] = $this->id_deuda->FormValue;
			} else {
				$sReturnUrl = "deuda_personalist.php"; // Return to list
			}
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "deuda_personalist.php"; // No matching record, return to list
					}
			}
			...
			$sReturnUrl = "deuda_personalist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
...
// Create page object
if (!isset($deuda_persona_view)) $deuda_persona_view = new cdeuda_persona_view();

// Page init
$deuda_persona_view->Page_Init();

// Page main
$deuda_persona_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$deuda_persona_view->Page_Render();
?>
...
var CurrentForm = fdeuda_personaview = new ew_Form("fdeuda_personaview", "view");

// Form_CustomValidate event
fdeuda_personaview.Form_CustomValidate = 
...
fdeuda_personaview.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
fdeuda_personaview.Lists["x_id_persona"] = {"LinkField":"x_Id","Ajax":true,"AutoFill":false,"DisplayFields":["x_nombres","x_paterno","x_materno",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"personas"};
fdeuda_personaview.Lists["x_id_persona"].Data = "<?php echo $deuda_persona_view->id_persona->LookupFilterQuery(FALSE, "view") ?>";
fdeuda_personaview.Lists["x_id_deuda"] = {"LinkField":"x_Id","Ajax":true,"AutoFill":false,"DisplayFields":["x_cuenta","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"deudas"};
fdeuda_personaview.Lists["x_id_deuda"].Data = "<?php echo $deuda_persona_view->id_deuda->LookupFilterQuery(FALSE, "view") ?>";
fdeuda_personaview.Lists["x_id_tipopersona"] = {"LinkField":"x_Id","Ajax":true,"AutoFill":false,"DisplayFields":["x_nombre","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"tipo_persona"};
fdeuda_personaview.Lists["x_id_tipopersona"].Data = "<?php echo $deuda_persona_view->id_tipopersona->LookupFilterQuery(FALSE, "view") ?>";
...
<?php if ($deuda_persona->Export == "") { ?>
<div class="ewToolbar">
<?php $deuda_persona_view->ExportOptions->Render("body") ?>
<?php
	foreach ($deuda_persona_view->OtherOptions as &$option)
		$option->Render("body");
?>
<div class="clearfix"></div>
</div>
<?php } ?>
<?php $deuda_persona_view->ShowPageHeader(); ?>
<?php
$deuda_persona_view->ShowMessage();
?>
<form name="fdeuda_personaview" id="fdeuda_personaview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($deuda_persona_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $deuda_persona_view->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="deuda_persona">
<input type="hidden" name="modal" value="<?php echo intval($deuda_persona_view->IsModal) ?>">
<table class="table table-striped table-bordered table-hover table-condensed ewViewTable">
<?php if ($deuda_persona->id_persona->Visible) { // id_persona ?>
	<tr id="r_id_persona">
		<td class="col-sm-2"><span id="elh_deuda_persona_id_persona"><?php echo $deuda_persona->id_persona->FldCaption() ?></span></td>
...
<a<?php echo $deuda_persona->id_persona->LinkAttributes() ?>><?php echo $deuda_persona->id_persona->ViewValue ?></a>
<?php } else { ?>
<?php echo $deuda_persona->id_persona->ViewValue ?>
<?php } ?>
</span>
</span>
</td>
...
<?php if ($deuda_persona->id_deuda->Visible) { // id_deuda ?>
	<tr id="r_id_deuda">
		<td class="col-sm-2"><span id="elh_deuda_persona_id_deuda"><?php echo $deuda_persona->id_deuda->FldCaption() ?></span></td>
...
<a<?php echo $deuda_persona->id_deuda->LinkAttributes() ?>><?php echo $deuda_persona->id_deuda->ViewValue ?></a>
<?php } else { ?>
<?php echo $deuda_persona->id_deuda->ViewValue ?>
<?php } ?>
</span>
</span>
</td>
...
<?php if ($deuda_persona->id_tipopersona->Visible) { // id_tipopersona ?>
	<tr id="r_id_tipopersona">
		<td class="col-sm-2"><span id="elh_deuda_persona_id_tipopersona"><?php echo $deuda_persona->id_tipopersona->FldCaption() ?></span></td>
...
<?php echo $deuda_persona->id_tipopersona->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php if ($deuda_persona->Export == "") { ?>
<script type="text/javascript">
fdeuda_personaview.Init();
</script>
<?php } ?>
<?php
$deuda_persona_view->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($deuda_persona->Export == "") { ?>
...
<?php
$deuda_persona_view->Page_Terminate();
?>
