...
$emails_view = NULL; // Initialize page object first

class cemails_view extends cemails {
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["Id"] <> "") {
				...
				$sReturnUrl = "emailslist.php"; // Return to list
			}
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "emailslist.php"; // No matching record, return to list
					}
			}
			...
			$sReturnUrl = "emailslist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
	...
	function SetupExportOptions() {
		...
		$url = "";
		$item->Body = "<button id=\"emf_emails\" class=\"ewExportLink ewEmail\" title=\"" . $Language->Phrase("ExportToEmailText") . "\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_emails',hdr:ewLanguage.Phrase('ExportToEmailText'),f:document.femailsview,key:" . ew_ArrayToJsonAttr($this->RecKey) . ",sel:false" . $url . "});\">" . $Language->Phrase("ExportToEmail") . "</button>";
		$item->Visible = TRUE;
...
// Create page object
if (!isset($emails_view)) $emails_view = new cemails_view();

// Page init
$emails_view->Page_Init();

// Page main
$emails_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$emails_view->Page_Render();
?>
...
var CurrentForm = femailsview = new ew_Form("femailsview", "view");

// Form_CustomValidate event
femailsview.Form_CustomValidate = 
...
<?php if ($emails->Export == "") { ?>
<div class="ewToolbar">
<?php $emails_view->ExportOptions->Render("body") ?>
<?php
	foreach ($emails_view->OtherOptions as &$option)
		$option->Render("body");
?>
<div class="clearfix"></div>
</div>
<?php } ?>
<?php $emails_view->ShowPageHeader(); ?>
<?php
$emails_view->ShowMessage();
?>
<form name="femailsview" id="femailsview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($emails_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $emails_view->Token ?>">
<?php } ?>
