...
class cuserlevels_list extends cuserlevels {
	...
	function GetFilterList() {
		...
		if (EW_SEARCH_FILTER_OPTION == "Server") {
			$sSavedFilterList = isset($UserProfile) ? $UserProfile->GetSearchFilters(CurrentUserName(), "fuserlevelslistsrch") : "";
		} else {
			$sSavedFilterList = "";
		}
	...
	function RenderListOptions() {
		...
		if ($Security->CanEdit()) {
			if (ew_IsMobile())
				...
				$oListOpt->Body = "<a class=\"ewRowLink ewEdit\" title=\"" . $editcaption . "\" data-table=\"userlevels\" data-caption=\"" . $editcaption . "\" href=\"javascript:void(0);\" onclick=\"ew_ModalDialogShow({lnk:this,btn:'EditBtn',url:'" . ew_HtmlEncode($this->EditUrl) . "'});\">" . $Language->Phrase("EditLink") . "</a>";
		} else {
			$oListOpt->Body = "";
		}
		...
		if ($Security->CanAdd()) {
			if (ew_IsMobile())
				...
				$oListOpt->Body = "<a class=\"ewRowLink ewCopy\" title=\"" . $copycaption . "\" data-table=\"userlevels\" data-caption=\"" . $copycaption . "\" href=\"javascript:void(0);\" onclick=\"ew_ModalDialogShow({lnk:this,btn:'AddBtn',url:'" . ew_HtmlEncode($this->CopyUrl) . "'});\">" . $Language->Phrase("CopyLink") . "</a>";
		} else {
			$oListOpt->Body = "";
		}
	...
	function SetupOtherOptions() {
		...
		if (ew_IsMobile())
			...
			$item->Body = "<a class=\"ewAddEdit ewAdd\" title=\"" . $addcaption . "\" data-table=\"userlevels\" data-caption=\"" . $addcaption . "\" href=\"javascript:void(0);\" onclick=\"ew_ModalDialogShow({lnk:this,btn:'AddBtn',url:'" . ew_HtmlEncode($this->AddUrl) . "'});\">" . $Language->Phrase("AddLink") . "</a>";
		$item->Visible = ($this->AddUrl <> "" && $Security->CanAdd());
		$option = $options["action"];

		// Add multi delete
		$item = &$option->Add("multidelete");
		$item->Body = "<a class=\"ewAction ewMultiDelete\" title=\"" . ew_HtmlTitle($Language->Phrase("DeleteSelectedLink")) . "\" data-caption=\"" . ew_HtmlTitle($Language->Phrase("DeleteSelectedLink")) . "\" href=\"\" onclick=\"ew_SubmitAction(event,{f:document.fuserlevelslist,url:'" . $this->MultiDeleteUrl . "',msg:ewLanguage.Phrase('DeleteConfirmMsg')});return false;\">" . $Language->Phrase("DeleteSelectedLink") . "</a>";
		$item->Visible = ($Security->CanDelete());
...
fuserlevelslist.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;
...
<?php
	...
	if ($bSelectLimit) {
		if ($userlevels_list->TotalRecs <= 0)
			$userlevels_list->TotalRecs = $userlevels->ListRecordCount();
	} else {
	...
	if ($userlevels->CurrentAction == "" && $userlevels_list->TotalRecs == 0) {
		if (!$Security->CanList())
			$userlevels_list->setWarningMessage(ew_DeniedMsg());
...
<div class="box ewBox ewGrid<?php if ($userlevels_list->IsAddOrEdit()) { ?> ewGridAddEdit<?php } ?> userlevels">
<?php if ($userlevels->Export == "") { ?>
<div class="box-header ewGridUpperPanel">
<?php if ($userlevels->CurrentAction <> "gridadd" && $userlevels->CurrentAction <> "gridedit") { ?>
<form name="ewPagerForm" class="form-inline ewForm ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($userlevels_list->Pager)) $userlevels_list->Pager = new cPrevNextPager($userlevels_list->StartRec, $userlevels_list->DisplayRecs, $userlevels_list->TotalRecs, $userlevels_list->AutoHidePager) ?>
...
<?php } ?>
<input type="hidden" name="t" value="userlevels">
<div id="gmp_userlevels" class="<?php if (ew_IsResponsiveLayout()) { ?>table-responsive <?php } ?>ewGridMiddlePanel">
<?php if ($userlevels_list->TotalRecs > 0 || $userlevels->CurrentAction == "gridedit") { ?>
...
<?php if ($userlevels->userlevelid->Visible) { // userlevelid ?>
	<?php if ($userlevels->SortUrl($userlevels->userlevelid) == "") { ?>
		<th data-name="userlevelid" class="<?php echo $userlevels->userlevelid->HeaderCellClass() ?>"><div id="elh_userlevels_userlevelid" class="userlevels_userlevelid"><div class="ewTableHeaderCaption"><?php echo $userlevels->userlevelid->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="userlevelid" class="<?php echo $userlevels->userlevelid->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $userlevels->SortUrl($userlevels->userlevelid) ?>',1);"><div id="elh_userlevels_userlevelid" class="userlevels_userlevelid">
...
<?php if ($userlevels->userlevelname->Visible) { // userlevelname ?>
	<?php if ($userlevels->SortUrl($userlevels->userlevelname) == "") { ?>
		<th data-name="userlevelname" class="<?php echo $userlevels->userlevelname->HeaderCellClass() ?>"><div id="elh_userlevels_userlevelname" class="userlevels_userlevelname"><div class="ewTableHeaderCaption"><?php echo $userlevels->userlevelname->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="userlevelname" class="<?php echo $userlevels->userlevelname->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $userlevels->SortUrl($userlevels->userlevelname) ?>',1);"><div id="elh_userlevels_userlevelname" class="userlevels_userlevelname">
