...
$deuda_persona_list = NULL; // Initialize page object first

class cdeuda_persona_list extends cdeuda_persona {
	...
	// Page object name
	var $PageObjName = 'deuda_persona_list';

	// Grid form hidden field names
	var $FormName = 'fdeuda_personalist';
	...
	function __construct() {
		...
		$this->MultiDeleteUrl = "deuda_personadelete.php";
		$this->MultiUpdateUrl = "deuda_personaupdate.php";
		...
		$this->FilterOptions->TagClassName = "ewFilterOption fdeuda_personalistsrch";
	...
	function GetFilterList() {
		...
		if (EW_SEARCH_FILTER_OPTION == "Server") {
			$sSavedFilterList = isset($UserProfile) ? $UserProfile->GetSearchFilters(CurrentUserName(), "fdeuda_personalistsrch") : "";
		} else {
			$sSavedFilterList = "";
		}
	...
	function ProcessFilterList() {
		...
		if (@$_POST["ajax"] == "savefilters") { // Save filter request (Ajax)
			...
			$UserProfile->SetSearchFilters(CurrentUserName(), "fdeuda_personalistsrch", $filters);
	...
	function RestoreFilterList() {
		...
		// Field id_deuda
		$this->id_deuda->AdvancedSearch->SearchValue = @$filter["x_id_deuda"];
		$this->id_deuda->AdvancedSearch->SearchOperator = @$filter["z_id_deuda"];
		$this->id_deuda->AdvancedSearch->SearchCondition = @$filter["v_id_deuda"];
		$this->id_deuda->AdvancedSearch->SearchValue2 = @$filter["y_id_deuda"];
		$this->id_deuda->AdvancedSearch->SearchOperator2 = @$filter["w_id_deuda"];
		$this->id_deuda->AdvancedSearch->Save();

		// Field id_tipopersona
		$this->id_tipopersona->AdvancedSearch->SearchValue = @$filter["x_id_tipopersona"];
		$this->id_tipopersona->AdvancedSearch->SearchOperator = @$filter["z_id_tipopersona"];
		$this->id_tipopersona->AdvancedSearch->SearchCondition = @$filter["v_id_tipopersona"];
	...
	function RenderListOptions() {
		...
		$oListOpt->Body = "<input type=\"checkbox\" name=\"key_m[]\" class=\"ewMultiSelect\" value=\"" . ew_HtmlEncode($this->id_persona->CurrentValue . $GLOBALS["EW_COMPOSITE_KEY_SEPARATOR"] . $this->id_deuda->CurrentValue) . "\" onclick=\"ew_ClickMultiCheckbox(event);\">";
	...
	function SetupOtherOptions() {
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"fdeuda_personalistsrch\" href=\"#\">" . $Language->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = TRUE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"fdeuda_personalistsrch\" href=\"#\">" . $Language->Phrase("DeleteFilter") . "</a>";
		$item->Visible = TRUE;
		...
		$options = &$this->OtherOptions;
			...
			foreach ($this->ListActions->Items as $listaction) {
				if ($listaction->Select == EW_ACTION_MULTIPLE) {
					...
					$item->Body = "<a class=\"ewAction ewListAction\" title=\"" . ew_HtmlEncode($caption) . "\" data-caption=\"" . ew_HtmlEncode($caption) . "\" href=\"\" onclick=\"ew_SubmitAction(event,jQuery.extend({f:document.fdeuda_personalist}," . $listaction->ToJson(TRUE) . "));return false;\">" . $icon . "</a>";
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $Language->Phrase("SearchPanel") . "\" data-caption=\"" . $Language->Phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fdeuda_personalistsrch\">" . $Language->Phrase("SearchLink") . "</button>";
		$item->Visible = TRUE;
	...
	function SetupExportOptions() {
		...
		$url = "";
		$item->Body = "<button id=\"emf_deuda_persona\" class=\"ewExportLink ewEmail\" title=\"" . $Language->Phrase("ExportToEmailText") . "\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_deuda_persona',hdr:ewLanguage.Phrase('ExportToEmailText'),f:document.fdeuda_personalist,sel:false" . $url . "});\">" . $Language->Phrase("ExportToEmail") . "</button>";
		$item->Visible = TRUE;
	...
	function ExportData() {
		...
		if (EW_EXPORT_MASTER_RECORD && $this->GetMasterFilter() <> "" && $this->getCurrentMasterTable() == "deudas") {
			...
			$rsmaster = $deudas->LoadRs($this->DbMasterFilter); // Load master record
			if ($rsmaster && !$rsmaster->EOF) {
				$ExportStyle = $Doc->Style;
				$Doc->SetStyle("v"); // Change to vertical
				if ($this->Export <> "csv" || EW_EXPORT_MASTER_RECORD_FOR_CSV) {
...
// Create page object
if (!isset($deuda_persona_list)) $deuda_persona_list = new cdeuda_persona_list();

// Page init
$deuda_persona_list->Page_Init();

// Page main
$deuda_persona_list->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$deuda_persona_list->Page_Render();
?>
...
var CurrentForm = fdeuda_personalist = new ew_Form("fdeuda_personalist", "list");
fdeuda_personalist.FormKeyCountName = '<?php echo $deuda_persona_list->FormKeyCountName ?>';

// Form_CustomValidate event
fdeuda_personalist.Form_CustomValidate = 
...
fdeuda_personalist.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
fdeuda_personalist.Lists["x_id_persona"] = {"LinkField":"x_Id","Ajax":true,"AutoFill":false,"DisplayFields":["x_nombres","x_paterno","x_materno",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"personas"};
fdeuda_personalist.Lists["x_id_persona"].Data = "<?php echo $deuda_persona_list->id_persona->LookupFilterQuery(FALSE, "list") ?>";
fdeuda_personalist.Lists["x_id_deuda"] = {"LinkField":"x_Id","Ajax":true,"AutoFill":false,"DisplayFields":["x_cuenta","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"deudas"};
fdeuda_personalist.Lists["x_id_deuda"].Data = "<?php echo $deuda_persona_list->id_deuda->LookupFilterQuery(FALSE, "list") ?>";
fdeuda_personalist.Lists["x_id_tipopersona"] = {"LinkField":"x_Id","Ajax":true,"AutoFill":false,"DisplayFields":["x_nombre","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"tipo_persona"};
fdeuda_personalist.Lists["x_id_tipopersona"].Data = "<?php echo $deuda_persona_list->id_tipopersona->LookupFilterQuery(FALSE, "list") ?>";

// Form object for search
var CurrentSearchForm = fdeuda_personalistsrch = new ew_Form("fdeuda_personalistsrch");

// Init search panel as collapsed
if (fdeuda_personalistsrch) fdeuda_personalistsrch.InitSearchPanel = true;
</script>
<script type="text/javascript">
...
<?php if ($deuda_persona->Export == "") { ?>
<div class="ewToolbar">
<?php if ($deuda_persona_list->TotalRecs > 0 && $deuda_persona_list->ExportOptions->Visible()) { ?>
<?php $deuda_persona_list->ExportOptions->Render("body") ?>
<?php } ?>
<?php if ($deuda_persona_list->SearchOptions->Visible()) { ?>
<?php $deuda_persona_list->SearchOptions->Render("body") ?>
<?php } ?>
<?php if ($deuda_persona_list->FilterOptions->Visible()) { ?>
<?php $deuda_persona_list->FilterOptions->Render("body") ?>
<?php } ?>
...
<?php } ?>
<?php
	$bSelectLimit = $deuda_persona_list->UseSelectLimit;
	if ($bSelectLimit) {
		if ($deuda_persona_list->TotalRecs <= 0)
			$deuda_persona_list->TotalRecs = $deuda_persona->ListRecordCount();
	} else {
		if (!$deuda_persona_list->Recordset && ($deuda_persona_list->Recordset = $deuda_persona_list->LoadRecordset()))
			$deuda_persona_list->TotalRecs = $deuda_persona_list->Recordset->RecordCount();
	}
	$deuda_persona_list->StartRec = 1;
	if ($deuda_persona_list->DisplayRecs <= 0 || ($deuda_persona->Export <> "" && $deuda_persona->ExportAll)) // Display all records
		$deuda_persona_list->DisplayRecs = $deuda_persona_list->TotalRecs;
	if (!($deuda_persona->Export <> "" && $deuda_persona->ExportAll))
		$deuda_persona_list->SetupStartRec(); // Set up start record position
	if ($bSelectLimit)
		$deuda_persona_list->Recordset = $deuda_persona_list->LoadRecordset($deuda_persona_list->StartRec-1, $deuda_persona_list->DisplayRecs);

	// Set no record found message
	if ($deuda_persona->CurrentAction == "" && $deuda_persona_list->TotalRecs == 0) {
		if (!$Security->CanList())
			$deuda_persona_list->setWarningMessage(ew_DeniedMsg());
		if ($deuda_persona_list->SearchWhere == "0=101")
			$deuda_persona_list->setWarningMessage($Language->Phrase("EnterSearchCriteria"));
		else
			$deuda_persona_list->setWarningMessage($Language->Phrase("NoRecord"));
	}
$deuda_persona_list->RenderOtherOptions();
?>
<?php if ($Security->CanSearch()) { ?>
<?php if ($deuda_persona->Export == "" && $deuda_persona->CurrentAction == "") { ?>
<form name="fdeuda_personalistsrch" id="fdeuda_personalistsrch" class="form-inline ewForm ewExtSearchForm" action="<?php echo ew_CurrentPage() ?>">
<?php $SearchPanelClass = ($deuda_persona_list->SearchWhere <> "") ? " in" : ""; ?>
<div id="fdeuda_personalistsrch_SearchPanel" class="ewSearchPanel collapse<?php echo $SearchPanelClass ?>">
<input type="hidden" name="cmd" value="search">
<input type="hidden" name="t" value="deuda_persona">
	<div class="ewBasicSearch">
<div id="xsr_1" class="ewRow">
	<div class="ewQuickSearch input-group">
	<input type="text" name="<?php echo EW_TABLE_BASIC_SEARCH ?>" id="<?php echo EW_TABLE_BASIC_SEARCH ?>" class="form-control" value="<?php echo ew_HtmlEncode($deuda_persona_list->BasicSearch->getKeyword()) ?>" placeholder="<?php echo ew_HtmlEncode($Language->Phrase("Search")) ?>">
	<input type="hidden" name="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" id="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" value="<?php echo ew_HtmlEncode($deuda_persona_list->BasicSearch->getType()) ?>">
	<div class="input-group-btn">
		<button type="button" data-toggle="dropdown" class="btn btn-default"><span id="searchtype"><?php echo $deuda_persona_list->BasicSearch->getTypeNameShort() ?></span><span class="caret"></span></button>
		<ul class="dropdown-menu pull-right" role="menu">
			<li<?php if ($deuda_persona_list->BasicSearch->getType() == "") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this)"><?php echo $Language->Phrase("QuickSearchAuto") ?></a></li>
			<li<?php if ($deuda_persona_list->BasicSearch->getType() == "=") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'=')"><?php echo $Language->Phrase("QuickSearchExact") ?></a></li>
			<li<?php if ($deuda_persona_list->BasicSearch->getType() == "AND") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'AND')"><?php echo $Language->Phrase("QuickSearchAll") ?></a></li>
			<li<?php if ($deuda_persona_list->BasicSearch->getType() == "OR") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'OR')"><?php echo $Language->Phrase("QuickSearchAny") ?></a></li>
		</ul>
...
<?php } ?>
<?php } ?>
<?php $deuda_persona_list->ShowPageHeader(); ?>
<?php
$deuda_persona_list->ShowMessage();
?>
<?php if ($deuda_persona_list->TotalRecs > 0 || $deuda_persona->CurrentAction <> "") { ?>
<div class="box ewBox ewGrid<?php if ($deuda_persona_list->IsAddOrEdit()) { ?> ewGridAddEdit<?php } ?> deuda_persona">
<?php if ($deuda_persona->Export == "") { ?>
<div class="box-header ewGridUpperPanel">
<?php if ($deuda_persona->CurrentAction <> "gridadd" && $deuda_persona->CurrentAction <> "gridedit") { ?>
<form name="ewPagerForm" class="form-inline ewForm ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($deuda_persona_list->Pager)) $deuda_persona_list->Pager = new cPrevNextPager($deuda_persona_list->StartRec, $deuda_persona_list->DisplayRecs, $deuda_persona_list->TotalRecs, $deuda_persona_list->AutoHidePager) ?>
<?php if ($deuda_persona_list->Pager->RecordCount > 0 && $deuda_persona_list->Pager->Visible) { ?>
<div class="ewPager">
<span><?php echo $Language->Phrase("Page") ?>&nbsp;</span>
<div class="ewPrevNext"><div class="input-group">
<div class="input-group-btn">
<!--first page button-->
	<?php if ($deuda_persona_list->Pager->FirstButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $deuda_persona_list->PageUrl() ?>start=<?php echo $deuda_persona_list->Pager->FirstButton->Start ?>"><span class="icon-first ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerFirst") ?>"><span class="icon-first ewIcon"></span></a>
	<?php } ?>
<!--previous page button-->
	<?php if ($deuda_persona_list->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $deuda_persona_list->PageUrl() ?>start=<?php echo $deuda_persona_list->Pager->PrevButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerPrevious") ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } ?>
</div>
<!--current page number-->
	<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $deuda_persona_list->Pager->CurrentPage ?>">
<div class="input-group-btn">
<!--next page button-->
	<?php if ($deuda_persona_list->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $deuda_persona_list->PageUrl() ?>start=<?php echo $deuda_persona_list->Pager->NextButton->Start ?>"><span class="icon-next ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerNext") ?>"><span class="icon-next ewIcon"></span></a>
	<?php } ?>
<!--last page button-->
	<?php if ($deuda_persona_list->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $deuda_persona_list->PageUrl() ?>start=<?php echo $deuda_persona_list->Pager->LastButton->Start ?>"><span class="icon-last ewIcon"></span></a>
	<?php } else { ?>
...
<span>&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $deuda_persona_list->Pager->PageCount ?></span>
</div>
<div class="ewPager ewRec">
	<span><?php echo $Language->Phrase("Record") ?>&nbsp;<?php echo $deuda_persona_list->Pager->FromIndex ?>&nbsp;<?php echo $Language->Phrase("To") ?>&nbsp;<?php echo $deuda_persona_list->Pager->ToIndex ?>&nbsp;<?php echo $Language->Phrase("Of") ?>&nbsp;<?php echo $deuda_persona_list->Pager->RecordCount ?></span>
</div>
<?php } ?>
</form>
<?php } ?>
...
<?php
	foreach ($deuda_persona_list->OtherOptions as &$option)
		$option->Render("body");
?>
</div>
<div class="clearfix"></div>
...
<?php } ?>
<form name="fdeuda_personalist" id="fdeuda_personalist" class="form-inline ewForm ewListForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($deuda_persona_list->CheckToken) { ?>
...
<table id="tbl_deuda_personalist" class="table ewTable">
<thead>
	<tr class="ewTableHeader">
<?php

// Header row
$deuda_persona_list->RowType = EW_ROWTYPE_HEADER;

// Render list options
$deuda_persona_list->RenderListOptions();

// Render list options (header, left)
$deuda_persona_list->ListOptions->Render("header", "left");
?>
<?php if ($deuda_persona->id_persona->Visible) { // id_persona ?>
	...
	<?php } else { ?>
		<th data-name="id_persona" class="<?php echo $deuda_persona->id_persona->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $deuda_persona->SortUrl($deuda_persona->id_persona) ?>',1);"><div id="elh_deuda_persona_id_persona" class="deuda_persona_id_persona">
...
<?php if ($deuda_persona->id_deuda->Visible) { // id_deuda ?>
	...
	<?php } else { ?>
		<th data-name="id_deuda" class="<?php echo $deuda_persona->id_deuda->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $deuda_persona->SortUrl($deuda_persona->id_deuda) ?>',1);"><div id="elh_deuda_persona_id_deuda" class="deuda_persona_id_deuda">
...
$deuda_persona_list->ListOptions->Render("header", "right");
?>
	</tr>
</thead>
<tbody>
<?php
if ($deuda_persona->ExportAll && $deuda_persona->Export <> "") {
	$deuda_persona_list->StopRec = $deuda_persona_list->TotalRecs;
} else {

	// Set the last record to display
	if ($deuda_persona_list->TotalRecs > $deuda_persona_list->StartRec + $deuda_persona_list->DisplayRecs - 1)
		$deuda_persona_list->StopRec = $deuda_persona_list->StartRec + $deuda_persona_list->DisplayRecs - 1;
	else
		$deuda_persona_list->StopRec = $deuda_persona_list->TotalRecs;
}
$deuda_persona_list->RecCnt = $deuda_persona_list->StartRec - 1;
if ($deuda_persona_list->Recordset && !$deuda_persona_list->Recordset->EOF) {
	$deuda_persona_list->Recordset->MoveFirst();
	$bSelectLimit = $deuda_persona_list->UseSelectLimit;
	if (!$bSelectLimit && $deuda_persona_list->StartRec > 1)
		$deuda_persona_list->Recordset->Move($deuda_persona_list->StartRec - 1);
} elseif (!$deuda_persona->AllowAddDeleteRow && $deuda_persona_list->StopRec == 0) {
	$deuda_persona_list->StopRec = $deuda_persona->GridAddRowCount;
}

// Initialize aggregate
$deuda_persona->RowType = EW_ROWTYPE_AGGREGATEINIT;
$deuda_persona->ResetAttrs();
$deuda_persona_list->RenderRow();
while ($deuda_persona_list->RecCnt < $deuda_persona_list->StopRec) {
	$deuda_persona_list->RecCnt++;
	if (intval($deuda_persona_list->RecCnt) >= intval($deuda_persona_list->StartRec)) {
		$deuda_persona_list->RowCnt++;

		// Set up key count
		$deuda_persona_list->KeyCount = $deuda_persona_list->RowIndex;
		...
		if ($deuda_persona->CurrentAction == "gridadd") {
			...
			$deuda_persona_list->LoadRowValues($deuda_persona_list->Recordset); // Load row values
		}
		$deuda_persona->RowType = EW_ROWTYPE_VIEW; // Render view

		// Set up row id / data-rowindex
		$deuda_persona->RowAttrs = array_merge($deuda_persona->RowAttrs, array('data-rowindex'=>$deuda_persona_list->RowCnt, 'id'=>'r' . $deuda_persona_list->RowCnt . '_deuda_persona', 'data-rowtype'=>$deuda_persona->RowType));
...
$deuda_persona_list->ListOptions->Render("body", "left", $deuda_persona_list->RowCnt);
?>
	<?php if ($deuda_persona->id_persona->Visible) { // id_persona ?>
		<td data-name="id_persona"<?php echo $deuda_persona->id_persona->CellAttributes() ?>>
<span id="el<?php echo $deuda_persona_list->RowCnt ?>_deuda_persona_id_persona" class="deuda_persona_id_persona">
...
<span id="el<?php echo $deuda_persona_list->RowCnt ?>_deuda_persona_id_deuda" class="deuda_persona_id_deuda">
...
$deuda_persona_list->ListOptions->Render("body", "right", $deuda_persona_list->RowCnt);
?>
	</tr>
<?php
	}
	if ($deuda_persona->CurrentAction <> "gridadd")
		$deuda_persona_list->Recordset->MoveNext();
}
?>
</tbody>
</table>
<?php } ?>
<?php if ($deuda_persona->CurrentAction == "") { ?>
...
// Close recordset
if ($deuda_persona_list->Recordset)
	$deuda_persona_list->Recordset->Close();
?>
<?php if ($deuda_persona->Export == "") { ?>
...
<?php } ?>
<?php if ($deuda_persona_list->TotalRecs == 0 && $deuda_persona->CurrentAction == "") { // Show other options ?>
<div class="ewListOtherOptions">
<?php
	foreach ($deuda_persona_list->OtherOptions as &$option) {
		$option->ButtonClass = "";
...
<?php if ($deuda_persona->Export == "") { ?>
<script type="text/javascript">
fdeuda_personalistsrch.FilterList = <?php echo $deuda_persona_list->GetFilterList() ?>;
fdeuda_personalistsrch.Init();
fdeuda_personalist.Init();
</script>
<?php } ?>
<?php
$deuda_persona_list->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($deuda_persona->Export == "") { ?>
...
<?php
$deuda_persona_list->Page_Terminate();
?>
