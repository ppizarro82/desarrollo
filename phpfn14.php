...
function CurrentProjectID() {
	...
	return "{A36EA07C-DB7F-422A-9088-B007545008C2}";
}

// Get current export file name
function CurrentExportFile() {
	return $GLOBALS["gsExportFile"];
}
...
function ew_GetFileATag($fld, $fn) {
	...
	foreach ($wrkfiles as $wrkfile) {
		if ($wrkfile <> "") {
			if ($html <> "")
				$html .= "<br>";
			$attrs = array("href" => ew_FullUrl($wrkpath . $wrkfile, "href"));
...
function ew_GetFileUploadUrl($fld, $val, $resize = FALSE, $encrypt = EW_ENCRYPT_FILE_PATH, $urlencode = TRUE) {
	if (!ew_EmptyStr($val)) {
		...
		if ($encrypt) {
			$fn = "ewfile14.php?t=" . ew_Encrypt($fld->TblName, $key) ."&fn=" . ew_Encrypt($path . $val, $key);
			if ($resize)
				$fn .= "&width=" . $fld->ImageWidth . "&height=" . $fld->ImageHeight;
		} elseif ($resize) {
			$fn = "ewfile14.php?t=" . rawurlencode($fld->TblName) . "&fn=" . ew_Encrypt($path . $val, $key) .
				"&width=" . $fld->ImageWidth . "&height=" . $fld->ImageHeight;
		} else {
			if (ew_IsRemote($path)) // Allow remote file
				$fn = $path;
			else
				$fn = $urlencode ? ew_UrlEncodeFilePath($path) : $path;
			$fn .= $urlencode ? ew_UrlEncodeFilePath($val) : $val;
		}
		return $fn;
	} else {
		return "";
...
function ew_GetFileViewTag(&$fld, $val) {
	...
	if (!ew_EmptyStr($val)) {
		...
		foreach ($wrkfiles as $wrkfile) {
			...
			if ($fld->FldViewTag == "IMAGE" && ($fld->IsBlobImage || ew_IsImageFile($wrkfile))) { // Image
				if ($href == "" && !$fld->UseColorbox) {
					if ($fn <> "") {
						if (ew_IsLazy())
							$image = "<img class=\"ewImage ewLazy img-thumbnail\" alt=\"\" src=\"data:image/png;base64,R0lGODlhAQABAAD/ACwAAAAAAQABAAACADs=\" data-src=\"" . $fn . "\"" . $fld->ViewAttributes() . ">";
						else
					...
					if ($fn <> "") {
						if (ew_IsLazy())
							$image = "<a" . $fld->LinkAttributes() . "><img class=\"ewImage ewLazy img-thumbnail\" alt=\"\" src=\"data:image/png;base64,R0lGODlhAQABAAD/ACwAAAAAAQABAAACADs=\" data-src=\"" . $fn . "\"" . $fld->ViewAttributes() . "></a>";
						else
...
function ew_GetImageUrl($fld, $val, $resize = FALSE, $encrypt = EW_ENCRYPT_FILE_PATH, $urlencode = TRUE) {
	if (!ew_EmptyStr($val)) {
		if ($encrypt) {
			$key = EW_RANDOM_KEY . session_id();
			$fn = "ewfile14.php?t=" . ew_Encrypt($fld->TblName, $key) ."&fn=" . ew_Encrypt($val, $key);
			if ($resize)
				$fn .= "&width=" . $fld->ImageWidth . "&height=" . $fld->ImageHeight;
		} elseif ($resize) {
			$fn = "ewfile14.php?t=" . rawurlencode($fld->TblName) . "&fn=" . ew_UrlEncodeFilePath($val) .
				"&width=" . $fld->ImageWidth . "&height=" . $fld->ImageHeight;
		} else {
			$fn = $val;
			if ($urlencode)
...
function ew_IsLazy() {
	global $EW_LAZY_LOAD, $gsExport, $gsCustomExport;
	return $EW_LAZY_LOAD && ($gsExport == "" || $gsExport == "print" && ($gsCustomExport == "" || $gsCustomExport == "print"));
}
...
class cExportCsv extends cExportBase {
	...
	function Export() {
		...
		if (!EW_DEBUG_ENABLED && ob_get_length())
			ob_end_clean();
		header('Set-Cookie: fileDownload=true; path=/');
		header('Content-Type: text/csv' . ((EW_CHARSET <> "") ? ";charset=" . EW_CHARSET : ""));
	...
	function ExportField(&$fld) {
		if ($fld->FldDataType <> EW_DATATYPE_BLOB) {
			...
			if (is_null($ExportValue))
				$ExportValue = "<Null>";
			$this->XmlDoc->AddField($fld->FldParm(), $ExportValue);
		}
	}
...
//
class cExportJson extends cExportBase {
	var $Items;
	var $HasParent;
	private $_item;
	...
	function ExportTableHeader() {
		...
		if ($this->HasParent) {
			if (is_array($this->Items))
				$this->Items[ew_ConvertToUtf8($this->Table->TableName)] = array();
	...
	function EndExportRow($rowcnt = 0) {
		...
		if ($this->HasParent) {
			if (is_array($this->Items))
				$this->Items[ew_ConvertToUtf8($this->Table->TableName)][] = $this->_item;
			elseif (is_object($this->Items))
				$this->Items->{ew_ConvertToUtf8($this->Table->TableName)}[] = $this->_item;
		} else {
			if (is_array($this->Items))
			...
			elseif (is_object($this->Items))
				$this->Items = array($this->Items, $this->_item); // Convert to array
			else
				$this->Items = $this->_item;
		}
	}
	...
	function ExportField(&$fld) {
		if ($fld->FldDataType <> EW_DATATYPE_BLOB) {
			if ($fld->UploadMultiple)
				...
				$ExportValue = $fld->ExportValue();
			$this->_item->{ew_ConvertToUtf8($fld->FldName)} = is_string($ExportValue) ? ew_ConvertToUtf8($ExportValue) : $ExportValue;
		}
	}
	...
	function Export() {
		...
		//header('Content-Disposition: attachment; filename=' . $gsExportFile . '.json');
		echo json_encode($this->Items, (EW_DEBUG_ENABLED) ? JSON_PRETTY_PRINT : 0);
	}
}
...
class cTableBase {
	...
	var $Visible = TRUE;
	var $fields = array();
	var $Rows = array(); // Data for Custom Template
	...
	function FieldCellAttributes() {
		$values = array();
		foreach ($this->fields as $fldname => $fld)
			$values[$fld->FldParm()] = $fld->CellAttributes();
		return $values;
	}

	// Get field DB values for Custom Template
	function CustomTemplateFieldValues() {
		global $EW_CUSTOM_TEMPLATE_DATATYPES, $EW_DATA_STRING_MAX_LENGTH;
		$values = array();
		foreach ($this->fields as $fldname => $fld) {
			if (in_array($fld->FldDataType, $EW_CUSTOM_TEMPLATE_DATATYPES)) {
				if (is_string($fld->DbValue) && strlen($fld->DbValue) > $EW_DATA_STRING_MAX_LENGTH)
					$values[$fld->FldParm()] = substr($fld->DbValue, 0, $EW_DATA_STRING_MAX_LENGTH);
				else
					$values[$fld->FldParm()] = $fld->DbValue;
			}
		}
		return $values;
	}
...
class cTable extends cTableBase {
	...
	// Highlight keyword
	private function Highlight($kwlist, $src) {
		$pattern = '';
		foreach ($kwlist as $kw)
			$pattern .= ($pattern == '' ? '' : '|') . preg_quote($kw, '/');
		if ($pattern == '')
			return $src;
		$pattern = '/(' . $pattern . ')/' . (strtolower(EW_CHARSET) == 'utf-8' ? 'u' : '') . (EW_HIGHLIGHT_COMPARE ? 'i' : '');
		$src = preg_replace_callback(
			$pattern,
			function($match) {
				return '<span class="' . $this->HighlightName() . ' ewHighlightSearch">' . $match[0] . '</span>';
			},
			$src
		);
		return $src;
...
class cField {
	...
	var $HeaderCellCssClass = ""; // Header cell (<th>) CSS class 
	var $FooterCellCssClass = ""; // Footer cell (<td> in <tfoot>) CSS class
	...
	var $HrefPath = EW_UPLOAD_HREF_PATH; // Href path (for download)
	...
	function FldParm() {
		return substr($this->FldVar, 2); // Remove "x_"
	}
	...
	function FldCaption() {
		...
		if ($this->Caption <> "")
			...
			return $Language->FieldPhrase($this->TblVar, $this->FldParm(), "FldCaption");
	}

	// Field title
	function FldTitle() {
		global $Language;
		return $Language->FieldPhrase($this->TblVar, $this->FldParm(), "FldTitle");
	}

	// Field image alt
	function FldAlt() {
		global $Language;
		return $Language->FieldPhrase($this->TblVar, $this->FldParm(), "FldAlt");
	}

	// Field error message
	function FldErrMsg() {
		global $Language;
		$err = $Language->FieldPhrase($this->TblVar, $this->FldParm(), "FldErrMsg");
	...
	function FldTagValue($i) {
		global $Language;
		return $Language->FieldPhrase($this->TblVar, $this->FldParm(), "FldTagValue" . $i);
	}

	// Field option caption
	function FldTagCaption($i) {
		global $Language;
		return $Language->FieldPhrase($this->TblVar, $this->FldParm(), "FldTagCaption" . $i);
	}

	// Set field visibility
	function SetVisibility() {
		$this->Visible = $GLOBALS[$this->TblVar]->SetFieldVisibility($this->FldParm());
	}
	...
	// Href path
	function HrefPath() {
		$path = ew_UploadPathEx(FALSE, ($this->HrefPath <> "") ? $this->HrefPath : $this->UploadPath);
	...
	function PhysicalUploadPath() {
		return ew_ServerMapPath($this->UploadPath);
	}	

	// Old Physical upload path
	function OldPhysicalUploadPath() {
		return ew_ServerMapPath($this->OldUploadPath);
	}
	...
	function EditAttributes() {
		...
		if ($this->ReadOnly) {
			if (in_array($this->FldHtmlTag, array("TEXT", "PASSWORD", "TEXTAREA"))) { // Elements support readonly
				$editattrs["readonly"] = TRUE;
			} else { // Elements do not support readonly
				$editattrs["disabled"] = TRUE;
				$editattrs["data-readonly"] = "1";
				ew_AppendClass($editattrs["class"], "disabled");
			}
		}
		$sAtt = "";
	...
	function HeaderCellClass() {
		$class = "ewTableHeaderCell";
		ew_AppendClass($class, $this->HeaderCellCssClass);
		return $class;
	}

	// Footer cell CSS class
	function FooterCellClass() {
		$class = "ewTableFooterCell";
		ew_AppendClass($class, $this->FooterCellCssClass);
		return $class;
	}

	// Add CSS class to all cells
	function AddClass($class) {
		ew_AppendClass($this->CellCssClass, $class);
		ew_AppendClass($this->HeaderCellCssClass, $class);
		ew_AppendClass($this->FooterCellCssClass, $class);
	}

	// Remove CSS class from all cells
	function RemoveClass($class) {
		ew_RemoveClass($this->CellCssClass, $class);
		ew_RemoveClass($this->HeaderCellCssClass, $class);
		ew_RemoveClass($this->FooterCellCssClass, $class);
	}
				...
				if (!$this->UploadMultiple) {
					...
					if (file_exists($imagefn)) {
						if ($this->ImageResize) {
							...
							if (ew_IsRemote($imagefn))
								return ew_TmpImage(file_get_contents($imagefn));
							else
						...
						if ($tmpfile <> "") {
							...
							if ($this->ImageResize) {
								...
								if ($tmpimage <> "")
									$tmpimage .= ",";
								if (ew_IsRemote($imagefn))
									$tmpimage .= ew_TmpImage(file_get_contents($imagefn));
								else
...
class cListOptions {
	...
	function RenderDropDownButton($body, $pos) {
		...
		foreach ($matches as $match) {
			...
			if ($this->UseImageAndText && $caption <> '') { // Image and text
...
class cListOption {
	...
	function Render($Part, $ColSpan = 1) {
		...
		if ($this->Parent->UseButtonGroup && $this->ShowInButtonGroup)
			ew_AppendClass($attrs["class"], "text-nowrap");
...
class cSubPages {
	...
	var $Parent = "false"; // For $Style = "" only, if a selector is provided, then all collapsible elements under the specified parent will be closed when a collapsible item is shown. 
	var $Items = array();
	var $ValidKeys = NULL;
	var $ActiveIndex = "";
...
class cBasicSearch {
	...
	// Get keyword list
	function KeywordList($Default = FALSE) {
		$sSearchKeyword = ($Default) ? $this->KeywordDefault : $this->Keyword;
		$sSearchType = ($Default) ? $this->TypeDefault : $this->Type;
		if ($sSearchKeyword <> "") {
...
class cUpload {
	...
	var $Parent; // Parent field object
	...
	function UploadFile() {
		...
		$fldvar = ($this->Index < 0) ? $this->Parent->FldVar : substr($this->Parent->FldVar, 0, 1) . $this->Index . substr($this->Parent->FldVar, 1);
		$wrkvar = "fn_" . $fldvar;
		...
		if (!$this->KeepFile && $this->FileName <> "" && !$this->UploadMultiple) {
			$f = ew_UploadTempPath($fldvar, $this->Parent->TblVar) . $this->FileName;
			if (file_exists($f)) {
		...
		if ($this->FileName <> "") {
			if ($this->UploadMultiple) {
				...
				if ($idx > -1 && $idx < count($ar)) {
					return ew_UploadTempPath($fldvar, $this->Parent->TblVar) . $ar[$idx];
				} else {
					$files = array();
					foreach ($ar as $fn)
						$files[] = ew_UploadTempPath($fldvar, $this->Parent->TblVar) . $fn;
					return $files;
				}
			} else {
				return ew_UploadTempPath($fldvar, $this->Parent->TblVar) . $this->FileName;
			}
		}
		return NULL;
	}
	...
	function SaveToFile($newFileName, $overWrite, $idx = -1) {
		$path = ew_ServerMapPath($this->UploadPath ?: $this->Parent->UploadPath);
		if (!ew_Empty($this->Value)) {
	...
	// Note: $quality is deprecated, kept for backward compatibility only.
	function ResizeAndSaveToFile($width, $height, $quality, $newFileName, $overWrite, $idx = -1) {
...
class cAdvancedSecurity {
	...
	function setCanAdd($b) {
		if ($b) {
			$this->CurrentUserLevel != EW_ALLOW_ADD;
		} else {
			$this->CurrentUserLevel &= ~EW_ALLOW_ADD;
		}
	}
	...
	function setCanDelete($b) {
		if ($b) {
			$this->CurrentUserLevel |= EW_ALLOW_DELETE;
		} else {
			$this->CurrentUserLevel &= ~EW_ALLOW_DELETE;
		}
	}
	...
	function setCanEdit($b) {
		if ($b) {
			$this->CurrentUserLevel |= EW_ALLOW_EDIT;
		} else {
			$this->CurrentUserLevel &= ~EW_ALLOW_EDIT;
		}
	}
	...
	function setCanView($b) {
		if ($b) {
			$this->CurrentUserLevel |= EW_ALLOW_VIEW;
		} else {
			$this->CurrentUserLevel &= ~EW_ALLOW_VIEW;
		}
	}
	...
	function setCanList($b) {
		if ($b) {
			$this->CurrentUserLevel |= EW_ALLOW_LIST;
		} else {
			$this->CurrentUserLevel &= ~EW_ALLOW_LIST;
		}
	}
	...
	function setCanReport($b) {
		if ($b) {
			$this->CurrentUserLevel |= EW_ALLOW_REPORT;
		} else {
			$this->CurrentUserLevel &= ~EW_ALLOW_REPORT;
		}
	}
	...
	function setCanSearch($b) {
		if ($b) {
			$this->CurrentUserLevel |= EW_ALLOW_SEARCH;
		} else {
			$this->CurrentUserLevel &= ~EW_ALLOW_SEARCH;
		}
	}
	...
	function setCanAdmin($b) {
		if ($b) {
			$this->CurrentUserLevel |= EW_ALLOW_ADMIN;
		} else {
			$this->CurrentUserLevel &= ~EW_ALLOW_ADMIN;
		}
	}
	...
	function AutoLogin() {
		...
		if (!$autologin && EW_ALLOW_LOGIN_BY_URL && isset($_GET["username"])) {
			$usr = ew_RemoveXSS($_GET["username"]);
			$pwd = ew_RemoveXSS(@$_GET["password"]);
	...
	function ValidateUser(&$usr, &$pwd, $autologin, $encrypted = FALSE, $provider = "") {
		global $Language, $UserProfile, $EW_AUTH_CONFIG;
		...
		if ($provider <> "") {
			...
			if (array_key_exists($provider, $providers) && $providers[$provider]["enabled"]) {
				try {
					...
					if (!array_key_exists("base_url", $EW_AUTH_CONFIG))
						$EW_AUTH_CONFIG["base_url"] = ew_FullUrl("hybridauth/", "auth"); // Set base URL
					$hybridauth = new Hybrid_Auth($EW_AUTH_CONFIG);
				...
				if (EW_DEBUG_ENABLED)
					die("Provider for " . $provider . " not found or not enabled.");
				return FALSE;
			}
		}
		...
		if (!$valid) {
			...
			if ($rs = $UserTableConn->Execute($sSql)) {
				if (!$rs->EOF) {
					$valid = $customValid || ew_ComparePassword($rs->fields('Password'), $pwd, $encrypted);
					if ($valid) {
						$_SESSION[EW_SESSION_STATUS] = "login";
		...
		if ($doc instanceof cXMLDocument) {
			...
			if (($projnode = $doc->SelectSingleNode("//configuration/project")) != NULL) {
				...
				if ($projid == $EW_RELATED_PROJECT_ID)
					$EW_RELATED_LANGUAGE_FOLDER = ew_IncludeTrailingDelimiter($doc->GetAttribute($projnode, "languagefolder"), TRUE);
			}
	...
	// Add user permission
	private function _addUserPermission($UserLevelName, $TableName, $UserPermission) {
	...
	function AddUserPermission($UserLevelName, $TableName, $UserPermission) {
		$arUserLevelName = is_array($UserLevelName) ? $UserLevelName : array($UserLevelName);
		$arTableName = is_array($TableName) ? $TableName : array($TableName);
		foreach ($arUserLevelName as $_userLevelName) {
			foreach ($arTableName as $_tableName) {
				$this->_addUserPermission($_userLevelName, $_tableName, $UserPermission);
			}
		}
	}

	// Delete user permission
	private function _deleteUserPermission($UserLevelName, $TableName, $UserPermission) {
	...
	function DeleteUserPermission($UserLevelName, $TableName, $UserPermission) {
		...
		foreach ($arUserLevelName as $_userLevelName) {
			foreach ($arTableName as $_tableName) {
				$this->_deleteUserPermission($_userLevelName, $_tableName, $UserPermission);
			}
		}
	}
	...
	function GetUserLevelID($UserLevelName) {
		...
		if (ew_SameStr($UserLevelName, "Administrator")) {
			return -1;
		} elseif ($Language && ew_SameStr($UserLevelName, $Language->Phrase("UserAdministrator"))) {
			return -1;
		} elseif (ew_SameStr($UserLevelName, "Default")) {
			return 0;
		} elseif ($Language && ew_SameStr($UserLevelName, $Language->Phrase("UserDefault"))) {
			return 0;
		} elseif ($UserLevelName <> "") {
			if (is_array($this->UserLevel)) {
				foreach ($this->UserLevel as $row) {
					list($levelid, $name) = $row;
					if (ew_SameStr($name, $UserLevelName))
						return $levelid;
				}
			}
		}
...
// Write HTTP header
function ew_Header($cache, $charset = EW_CHARSET, $json = FALSE) {
	$export = @$_GET["export"];
...
function ew_ContentExt($data) {
	...
	if ($ct) {
		foreach ($EW_MIME_TYPES as $ext => $mimetype) {
			if ($ct == $mimetype)
				return "." . $ext;
		}
	}
...
// Check if HTTP POST
function ew_IsPost() {
	return ew_SameText(ew_ServerVar("REQUEST_METHOD"), "POST");
}
...
function ew_WriteAuditTrail($pfx, $dt, $script, $usr, $action, $table, $field, $keyvalue, $oldvalue, $newvalue) {
	...
	if ($bWriteAuditTrail) {
		if (EW_AUDIT_TRAIL_TO_DATABASE) {
			...
			$filename = ew_ServerMapPath($sFolder) . $sFn;
...
function ew_UploadTempPath($fldvar = "", $tblvar = "") {
	if ($fldvar !== FALSE) { // Physical path
		$path = (EW_UPLOAD_TEMP_PATH && EW_UPLOAD_TEMP_HREF_PATH) ? ew_IncludeTrailingDelimiter(EW_UPLOAD_TEMP_PATH, TRUE) : ew_UploadPath(TRUE);
		if ($tblvar <> "" || $fldvar <> "")
			$path = ew_IncludeTrailingDelimiter($path . EW_UPLOAD_TEMP_FOLDER_PREFIX . session_id(), TRUE);
		if ($tblvar <> "")
		...
		return (EW_UPLOAD_TEMP_PATH && EW_UPLOAD_TEMP_HREF_PATH) ? ew_IncludeTrailingDelimiter(EW_UPLOAD_TEMP_HREF_PATH, FALSE) : ew_UploadPath(FALSE);
	}
}
...
function ew_RenderUploadField(&$fld, $idx = -1) {
	...
	if ($fld->FldDataType == EW_DATATYPE_BLOB) { // Blob field
		if (!ew_Empty($fld->Upload->DbValue)) {
			...
			// Create upload file
			$filename = ($fld->Upload->FileName <> "") ? $fld->Upload->FileName : $fld->FldParm();
		...
		if (!ew_Empty($fld->Upload->FileName)) {
			...
			$filepath = (@$pathinfo["dirname"] <> "" && @$pathinfo["dirname"] <> ".") ? ew_PathCombine($fld->UploadPath, $pathinfo["dirname"], !ew_IsRemote($fld->UploadPath)) : $fld->UploadPath;
			if ($fld->UploadMultiple)
...
function ew_CreateImageFromText($txt, $file, $width = EW_UPLOAD_THUMBNAIL_WIDTH, $height = 0, $font = EW_TMP_IMAGE_FONT) {
	...
	if (strrpos($font, '.') === FALSE)
		$font .= '.ttf';
	$font = ew_IncludeTrailingDelimiter($GLOBALS["EW_FONT_PATH"], TRUE) . $font; // Always use full path
...
function ew_CleanUploadTempPaths($sessionid = "") {
	$folder = (EW_UPLOAD_TEMP_PATH) ? ew_IncludeTrailingDelimiter(EW_UPLOAD_TEMP_PATH, TRUE) : ew_UploadPath(TRUE);
	if (@is_dir($folder)) {
...
function ew_SendEmail($sFrEmail, $sToEmail, $sCcEmail, $sBccEmail, $sSubject, $sMail, $sFormat, $sCharset, $sSmtpSecure = "", $arAttachments = array(), $arImages = array(), $arProperties = NULL) {
	...
	if (is_array($arImages)) {
		foreach ($arImages as $tmpimage) {
			$file = ew_UploadTempPath() . $tmpimage;
...
function ew_CleanEmailContent($Content) {
	$Content = preg_replace('/class="box ewGrid \w+"/', "", $Content);
...
function ew_UploadPathEx($PhyPath, $DestPath) {
	global $EW_ROOT_RELATIVE_PATH;
	if ($PhyPath) {
		$PhyPath = !ew_IsRemote($DestPath); // Not remote
		if ($PhyPath)
			$DestPath = str_replace("/", EW_PATH_DELIMITER, $DestPath);
...
// Global upload path relative to application root
// If PhyPath is TRUE(1), return physical path on the server
// If PhyPath is FALSE(0), return relative URL
...
function ew_ServerMapPath($path, $isFile = FALSE) {
	$pathinfo = ew_IsRemote($path) ? array() : pathinfo($path);
	if ($isFile || @$pathinfo["extension"] <> "") // File
		return ew_UploadPathEx(TRUE, $pathinfo["dirname"]) . $pathinfo["basename"];
	else // Folder
		return ew_UploadPathEx(TRUE, $path);
}
...
function ew_TmpFolder() {
	...
	if (EW_IS_WINDOWS) {
		...
		if (EW_UPLOAD_TMP_PATH <> "")
			$folders[] = ew_ServerMapPath(EW_UPLOAD_TMP_PATH);
		$folders[] = '/tmp';
	}
...
function ew_SaveFile($folder, $fn, $filedata) {
	...
	$res = FALSE;
	if (ew_CreateFolder($folder)) {
		if ($handle = fopen(ew_IncludeTrailingDelimiter($folder, TRUE) . $fn, 'w')) {
...
function ew_CopyFile($folder, $fn, $file) {
	...
	if (file_exists($file)) {
		if (ew_CreateFolder($folder)) {
			$newfile = ew_IncludeTrailingDelimiter($folder, TRUE) . $fn;
...
function ew_TmpImageLnk($file, $lnktype = "") {
	...
	if ($lnktype == "email" || $lnktype == "cid") {
		...
		// If EW_UPLOAD_TEMP_PATH, returns physical path, else returns relative path.
		return ew_UploadTempPath(EW_UPLOAD_TEMP_PATH && EW_UPLOAD_TEMP_HREF_PATH) . $file; 
	}
}
...
function ew_DeleteTmpImages() {
	...
	foreach ($gTmpImages as $tmpimage)
		@unlink(ew_UploadTempPath() . $tmpimage);
}
...
function ew_UrlAddQuery($url, $qry) {
	if (strval($qry) == "")
		return $url;
	return $url . (ew_ContainsStr($url, "?") ? "&" : "?") . $qry;
}

// Add "hash" parameter to URL
function ew_UrlAddHash($url, $hash) {
	return ew_UrlAddQuery($url, "hash=" . $hash);
}
?>
<?php
/**
...
function ew_ResizeFile($fn, $tn, &$width, &$height, $plugins = array()) {
	...
	if (!$info || !in_array($info[2], array(1, 2, 3)) || $width <= 0 && $height <= 0) {
		if ($fn <> $tn) copy($fn, $tn);
		return;
	}
...
function ew_ResizeFileToBinary($fn, &$width, &$height, $plugins = array()) {
	...
	if (!$info)
		return NULL;
	if (!in_array($info[2], array(1, 2, 3)) || $width <= 0 && $height <= 0) {
...
function ew_ComparePassword($pwd, $input, $encrypted = FALSE) {
	...
	if (preg_match('/^\$[HP]\$/', $pwd)) { // phpass
		include $EW_RELATIVE_PATH . "passwordhash.php";
...
function CurrentUserInfo($fldname) {
	...
	if (!is_null($value)) {
		...
		if (strval($user) <> "")
			return ew_ExecuteScalar("SELECT " . ew_QuotedName($fldname, EW_USER_TABLE_DBID) . " FROM " . EW_USER_TABLE . " WHERE " .
				str_replace("%u", ew_AdjustSql($user, EW_USER_TABLE_DBID), EW_USER_NAME_FILTER), EW_USER_TABLE_DBID);
	}
	return NULL;
}
...
function IsExport($format) {
	global $gsExport;
	return ew_SameText($gsExport, $format);
}
/**
...
function ew_SetDebugMsg($v, $newline = TRUE) {
	global $gsDebugMsg, $gTimer;
	$ar = preg_split('/<(hr|br)>/', trim($v));
	$ar = array_filter($ar, function($s) {
		return trim($s); 
	});
	$v = implode("; ", $ar);
	$gsDebugMsg .= "<p><samp>" . (isset($gTimer) ? number_format($gTimer->GetElapsedTime(), 6) . ": " : "") . $v . "</samp></p>";
}
...
function ew_SaveDebugMsg() {
	global $gsDebugMsg;
	if (EW_DEBUG_ENABLED)
		$_SESSION["EW_DEBUG_MESSAGE"] = $gsDebugMsg;
}

// Load global debug message
function ew_LoadDebugMsg() {
	global $gsDebugMsg;
	if (EW_DEBUG_ENABLED) {
		$gsDebugMsg = @$_SESSION["EW_DEBUG_MESSAGE"];
		$_SESSION["EW_DEBUG_MESSAGE"] = "";
	}
}
...
function ew_DeniedMsg() {
	global $Language;
	return str_replace("%s", ew_ScriptName(), $Language->Phrase("NoPermission"));
}
...
class cUserProfile {
	...
	function Assign($input) {
		if (is_object($input)) {
			...
			$input = array_filter($input, function($val) {
				...
				if (is_bool($val) || is_float($val) || is_int($val) || is_null($val) || is_string($val) && strlen($val) <= $EW_DATA_STRING_MAX_LENGTH)
					return TRUE;
				return FALSE;
			});
	...
	// Get search filters
	function GetSearchFilters($usr, $pageid) {
		$bSameProfile = CurrentUserName() == $usr;
		if (!$bSameProfile)
			$p = $this->Profile; // Backup current profile
		if ($this->LoadProfileFromDatabase($usr)) {
			try {
				$allfilters = @unserialize($this->Profile[EW_USER_PROFILE_SEARCH_FILTERS]);
				if (!$bSameProfile)
					$this->Profile = $p; // Restore current profile
	...
	// Set search filters
	function SetSearchFilters($usr, $pageid, $filters) {
		$bSameProfile = CurrentUserName() == $usr;
		if (!$bSameProfile)
			$p = $this->Profile; // Backup current profile
		if ($this->LoadProfileFromDatabase($usr)) {
			try {
				...
				$this->Profile[EW_USER_PROFILE_SEARCH_FILTERS] = serialize($allfilters);
				$this->SaveProfileToDatabase($usr);
				if (!$bSameProfile)
					$this->Profile = $p; // Restore current profile
				return TRUE;
	...
	function Load() {
		if (isset($_SESSION[EW_SESSION_USER_PROFILE]))
			$this->LoadProfile($_SESSION[EW_SESSION_USER_PROFILE]);
	}
...
function ew_CheckTime($value) {
	...
	if (strval($value) == "") return TRUE;
	return preg_match('/^(0[0-9]|1[0-9]|2[0-3])' . preg_quote($EW_TIME_SEPARATOR) . '[0-5][0-9](' . preg_quote($EW_TIME_SEPARATOR) . '[0-5][0-9](\.\d+)?)?$/', $value);
}
...
// Include shared code
include_once $EW_RELATIVE_PATH . "ewshared14.php";
?>
