...
$cuentas_edit = NULL; // Initialize page object first

class ccuentas_edit extends ccuentas {
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// denominacion
			$this->denominacion->SetDbValueDef($rsnew, $this->denominacion->CurrentValue, "", $this->denominacion->ReadOnly);

			// inicio_contrato
			$this->inicio_contrato->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->inicio_contrato->CurrentValue, 7), NULL, $this->inicio_contrato->ReadOnly);

			// fin_contrato
			$this->fin_contrato->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->fin_contrato->CurrentValue, 7), NULL, $this->fin_contrato->ReadOnly);
...
// Create page object
if (!isset($cuentas_edit)) $cuentas_edit = new ccuentas_edit();

// Page init
$cuentas_edit->Page_Init();

// Page main
$cuentas_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$cuentas_edit->Page_Render();
?>
...
var CurrentForm = fcuentasedit = new ew_Form("fcuentasedit", "edit");

// Validate form
fcuentasedit.Validate = function() {
...
fcuentasedit.Form_CustomValidate = 
...
fcuentasedit.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
fcuentasedit.Lists["x_estado"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fcuentasedit.Lists["x_estado"].Options = <?php echo json_encode($cuentas_edit->estado->Options()) ?>;
...
<?php $cuentas_edit->ShowPageHeader(); ?>
<?php
$cuentas_edit->ShowMessage();
?>
<form name="fcuentasedit" id="fcuentasedit" class="<?php echo $cuentas_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($cuentas_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $cuentas_edit->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="cuentas">
<input type="hidden" name="a_edit" id="a_edit" value="U">
<input type="hidden" name="modal" value="<?php echo intval($cuentas_edit->IsModal) ?>">
<div class="ewEditDiv"><!-- page* -->
<?php if ($cuentas->Id->Visible) { // Id ?>
	<div id="r_Id" class="form-group">
		<label id="elh_cuentas_Id" class="<?php echo $cuentas_edit->LeftColumnClass ?>"><?php echo $cuentas->Id->FldCaption() ?></label>
		<div class="<?php echo $cuentas_edit->RightColumnClass ?>"><div<?php echo $cuentas->Id->CellAttributes() ?>>
<span id="el_cuentas_Id">
<span<?php echo $cuentas->Id->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $cuentas->Id->EditValue ?></p></span>
</span>
<input type="hidden" data-table="cuentas" data-field="x_Id" name="x_Id" id="x_Id" value="<?php echo ew_HtmlEncode($cuentas->Id->CurrentValue) ?>">
<?php echo $cuentas->Id->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($cuentas->codigo->Visible) { // codigo ?>
	<div id="r_codigo" class="form-group">
		<label id="elh_cuentas_codigo" for="x_codigo" class="<?php echo $cuentas_edit->LeftColumnClass ?>"><?php echo $cuentas->codigo->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $cuentas_edit->RightColumnClass ?>"><div<?php echo $cuentas->codigo->CellAttributes() ?>>
...
<?php if ($cuentas->denominacion->Visible) { // denominacion ?>
	<div id="r_denominacion" class="form-group">
		<label id="elh_cuentas_denominacion" for="x_denominacion" class="<?php echo $cuentas_edit->LeftColumnClass ?>"><?php echo $cuentas->denominacion->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $cuentas_edit->RightColumnClass ?>"><div<?php echo $cuentas->denominacion->CellAttributes() ?>>
...
<?php if ($cuentas->inicio_contrato->Visible) { // inicio_contrato ?>
	<div id="r_inicio_contrato" class="form-group">
		<label id="elh_cuentas_inicio_contrato" for="x_inicio_contrato" class="<?php echo $cuentas_edit->LeftColumnClass ?>"><?php echo $cuentas->inicio_contrato->FldCaption() ?></label>
		<div class="<?php echo $cuentas_edit->RightColumnClass ?>"><div<?php echo $cuentas->inicio_contrato->CellAttributes() ?>>
...
<script type="text/javascript">
ew_CreateDateTimePicker("fcuentasedit", "x_inicio_contrato", {"ignoreReadonly":true,"useCurrent":false,"format":7});
</script>
<?php } ?>
</span>
...
<?php if ($cuentas->fin_contrato->Visible) { // fin_contrato ?>
	<div id="r_fin_contrato" class="form-group">
		<label id="elh_cuentas_fin_contrato" for="x_fin_contrato" class="<?php echo $cuentas_edit->LeftColumnClass ?>"><?php echo $cuentas->fin_contrato->FldCaption() ?></label>
		<div class="<?php echo $cuentas_edit->RightColumnClass ?>"><div<?php echo $cuentas->fin_contrato->CellAttributes() ?>>
...
<script type="text/javascript">
ew_CreateDateTimePicker("fcuentasedit", "x_fin_contrato", {"ignoreReadonly":true,"useCurrent":false,"format":7});
</script>
<?php } ?>
</span>
...
<?php if ($cuentas->estado->Visible) { // estado ?>
	<div id="r_estado" class="form-group">
		<label id="elh_cuentas_estado" class="<?php echo $cuentas_edit->LeftColumnClass ?>"><?php echo $cuentas->estado->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $cuentas_edit->RightColumnClass ?>"><div<?php echo $cuentas->estado->CellAttributes() ?>>
...
<?php
	if (in_array("deudas", explode(",", $cuentas->getCurrentDetailTable())) && $deudas->DetailEdit) {
?>
...
<?php } ?>
<?php if (!$cuentas_edit->IsModal) { ?>
<div class="form-group"><!-- buttons .form-group -->
	<div class="<?php echo $cuentas_edit->OffsetColumnClass ?>"><!-- buttons offset -->
<button class="btn btn-primary ewButton" name="btnAction" id="btnAction" type="submit"><?php echo $Language->Phrase("SaveBtn") ?></button>
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $cuentas_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div><!-- /buttons offset -->
